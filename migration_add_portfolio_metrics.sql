...
ADD COLUMN IF NOT EXISTS "pnlPercentage" numeric,
ADD COLUMN IF NOT EXISTS "winRate" numeric,
ADD COLUMN IF NOT EXISTS "profitFactor" numeric,
ADD COLUMN IF NOT EXISTS "winningTrades" integer,
ADD COLUMN IF NOT EXISTS "losingTrades" integer,
...
-- UPDATE public.portfolio_snapshots SET "pnlPercentage" = 0 WHERE "pnlPercentage" IS NULL;
...
-- UPDATE public.portfolio_snapshots SET "winningTrades" = 0 WHERE "winningTrades" IS NULL;
-- UPDATE public.portfolio_snapshots SET "losingTrades" = 0 WHERE "losingTrades" IS NULL;
