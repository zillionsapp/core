...
class SupabaseDataStore {
    constructor() {
        this.supabase = null;
        this.inMemoryTrades = [];
        if (env_1.config.SUPABASE_URL && env_1.config.SUPABASE_KEY) {
            this.supabase = (0, supabase_js_1.createClient)(env_1.config.SUPABASE_URL, env_1.config.SUPABASE_KEY);
        }
        else {
    ...
    async getTrades(symbol, limit, offset) {
        if (!this.supabase) {
    ...
    async getPortfolioSnapshots(limit = 50, period) {
        if (!this.supabase)
            return [];
        const now = Date.now();
    ...
    consolidateSnapshots(data, targetCount) {
    ...
    async saveRiskState(state) {
        ...
        if (error)
            console.error('Error saving risk state:', error);
    }
    async updateChartCache(period, data) {
        if (!this.supabase)
            return;
        const { error } = await this.supabase
        ...
        if (error)
            console.error(`Error updating chart cache for ${period}:`, error);
    }
    async getChartCache(period) {
        if (!this.supabase)
            return [];
        const { data, error } = await this.supabase
            .from('portfolio_chart_cache')
    ...
    async saveVaultTransaction(transaction) {
        if (!this.supabase)
            return;
        const { error } = await this.supabase.from('vault_transactions').insert(transaction);
        if (error)
            console.error('Error saving vault transaction:', error);
    }
    async getVaultTransactions(email) {
        if (!this.supabase)
            return [];
    ...
    async saveCommissionTransaction(transaction) {
        if (!this.supabase) {
        ...
        if (error) {
            ...
            if (vaultError)
                console.error('Error saving commission to vault_transactions:', vaultError);
        }
    }
    ...
    async getInvitedUsersCount(inviterId) {
        if (!this.supabase)
            return 0;
        const { count, error } = await this.supabase
            .from('invite_code_usages')
            .select('*', { count: 'exact', head: true })
            .eq('invite_code_id', this.supabase
            .from('invite_codes')
            .select('id')
    ...
    async getTotalVaultAssets() {
        ...
        return filtered.reduce((sum, t) => {
            ...
            if (t.type === 'COMMISSION_PAID')
                return sum + Number(t.amount); // amount is negative
            return sum;
        }, 0);
    }
}
