...
exports.ComparisonBacktestRunner = void 0;
const strategy_manager_1 = require("../core/strategy.manager");
const runner_1 = require("./runner");
const env_1 = require("../config/env");
class ComparisonBacktestRunner {
    constructor() {
        this.runner = new runner_1.BacktestRunner();
    }
    async runAll(symbol = env_1.config.STRATEGY_SYMBOL, interval = env_1.config.STRATEGY_INTERVAL, verbose = false) {
        const strategies = strategy_manager_1.StrategyManager.getAvailableStrategies();
        const results = [];
        console.log(`[Comparison Backtest] Running ${strategies.length} strategies on ${symbol} ${interval} with ${env_1.config.BACKTEST_CANDLE_COUNT} candles...`);
    ...
    generateReport(results, symbol, interval) {
        if (results.length === 0) {
...
exports.ComparisonBacktestRunner = ComparisonBacktestRunner;
// CLI runner
if (require.main === module) {
    const args = process.argv.slice(2);
    const symbol = args[0] || env_1.config.STRATEGY_SYMBOL;
    const interval = args[1] || env_1.config.STRATEGY_INTERVAL;
