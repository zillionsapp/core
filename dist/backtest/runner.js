...
const strategy_manager_1 = require("../core/strategy.manager");
const paper_1 = require("../adapters/exchange/paper");
const binance_public_1 = require("../adapters/data/binance_public");
const supabase_1 = require("../adapters/database/supabase");
const risk_manager_1 = require("../core/risk.manager");
const env_1 = require("../config/env");
const simulation_time_provider_1 = require("./simulation.time.provider");
class BacktestRunner {
    constructor() {
        this.activeTrade = null;
        // Shared Data Provider
        const publicData = new binance_public_1.BinancePublicData();
        this.db = new supabase_1.SupabaseDataStore();
        this.exchange = new paper_1.PaperExchange(publicData, undefined, undefined, this.db);
        this.timeProvider = new simulation_time_provider_1.SimulationTimeProvider();
        this.riskManager = new risk_manager_1.RiskManager(this.exchange, this.db, this.timeProvider);
    }
    async run(strategyName, symbol, interval, verbose = process.env.NODE_ENV !== 'test') {
        ...
        // 2. Get History
        const candles = await this.exchange.getCandles(symbol, interval, env_1.config.BACKTEST_CANDLE_COUNT);
        const strategy = strategy_manager_1.StrategyManager.getStrategy(strategyName);
        strategy.init({});
        let tradesCount = 0;
        let winningTrades = 0;
        ...
        for (let i = 50; i < candles.length; i++) {
            ...
            if (verbose)
                await this.logPortfolioState(symbol, currentCandle.close);
            // --- Risk Check (SL/TP) ---
            if (this.activeTrade) {
                let exitReason = '';
                ...
                if (exitReason) {
                    ...
                    try {
                        ...
                        if (verbose)
                            console.log(`[Backtest] ${exitReason} triggered at ${currentCandle.close} (High: ${currentCandle.high}, Low: ${currentCandle.low}) | ID: ${this.activeTrade.id}`);
                        this.activeTrade = null;
                    }
                    catch (e) {
                        console.error(e);
            ...
            if (!this.activeTrade) {
                ...
                if (signal && signal.action !== 'HOLD') {
                    try {
                        ...
                        if (verbose)
                            console.log(`[Backtest] ${signal.action} Entry. SL: ${this.activeTrade.stopLossPrice}, TP: ${this.activeTrade.takeProfitPrice}`);
                    }
                    catch (e) {
...
if (require.main === module) {
    ...
    const strategyName = args[0] || env_1.config.STRATEGY_NAME;
    const symbol = args[1] || env_1.config.STRATEGY_SYMBOL;
    const interval = args[2] || env_1.config.STRATEGY_INTERVAL;
